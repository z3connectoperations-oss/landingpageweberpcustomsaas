...
const projects = [
  {
    ...
    meta: "Stack: React, Node, Firebase",
...
export function ProofStack() {
  return (
    <section id="proof" className="py-20 md:py-28 bg-primary">
      <div className="w-full max-w-7xl mx-auto px-5 md:px-12 lg:px-20 xl:px-24 flex flex-col gap-16 md:gap-24">
        
        {/* Top: Stat bar */}
        <div>
          <div className="grid grid-cols-3 gap-4 md:gap-8 border-y border-divider py-8 md:py-12 divide-x divide-white/10">
            {stats.map((stat, idx) => (
              <div key={idx} className="flex flex-col items-center justify-center text-center">
                <span className="text-[42px] md:text-[64px] font-black text-accent mb-1 md:mb-2">{stat.value}</span>
                <span className="text-xs md:text-[14px] uppercase tracking-widest text-muted font-bold">{stat.label}</span>
              </div>
            ))}
          </div>
          <p className="text-center text-white/40 text-sm mt-6">Across manufacturing, healthcare, logistics &amp; construction</p>
        </div>

        {/* Bottom: Bento Grid Featured Products */}
        <div>
          <div className="text-center mb-10 md:mb-16">
            <p className="text-accent text-sm font-semibold uppercase tracking-widest mb-3">Featured Products</p>
            <h2 className="text-[28px] md:text-[48px] font-black text-white leading-tight">Work we&apos;ve shipped.</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {projects.map((p, idx) => (
              <div key={idx} className={`p-5 md:p-8 rounded-2xl flex flex-col gap-3 md:gap-4 border border-white/10 bg-[#0A0A0A]/50 backdrop-blur-md hover:border-accent/40 hover:bg-white/[0.07] transition-all duration-300 h-full ${p.size === 'large' ? 'lg:col-span-2 shadow-xl' : 'lg:col-span-1 shadow-lg'}`}>
                ...
                <div className="flex flex-wrap gap-1.5 md:gap-2 mt-auto pt-3 md:pt-4">
                  {p.tags.map(tag => (
                    <span key={tag} className="px-2 py-0.5 md:px-3 md:py-1.5 bg-white/5 rounded-full text-xs md:text-[12px] font-medium text-white/70 border border-white/5">
                      {tag}
                    </span>
                  ))}
                </div>
